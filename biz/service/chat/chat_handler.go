...
func (s *ChatService) handleChatMessage(userID int64, chatMessage *ws.ChatMessage) error {
	...
	if _, err := s.sendMessageToUser(userID, ws.MessageTypeChat, chatMessage); err != nil {
		return errors.Wrapf(err, "sendMessageToUser failed, userID=%d", userID)
	}

	receiverOnline, err := s.sendMessageToUser(chatMessage.ReceiverID, ws.MessageTypeChat, chatMessage)
	if err != nil {
